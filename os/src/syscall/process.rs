...
use crate::{
    config::{PAGE_SIZE, PAGE_SIZE_BITS}, loader::get_app_data_by_name, mm::{translated_byte_buffer, translated_refmut, translated_str, MapPermission, VirtPageNum}, task::{
...
pub fn sys_spawn(_path: *const u8) -> isize {
    ...
    if let Some(data) = get_app_data_by_name(path.as_str()) {
        ...
        println!("sys_spawn: create process {} on {}", new_task_pid, path.as_str());
        add_task(new_task);
        new_task_pid
    } else {
        return -1;
